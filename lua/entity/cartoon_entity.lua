...
local CartoonEntity = {}
CartoonEntity.__index = CartoonEntity


function CartoonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CartoonEntity)
  self._name = "cartoon"
  self._client = client
...
function CartoonEntity:get_name()
  return self._name
end


function CartoonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CartoonEntity.new(self._client, opts)
end
...
function CartoonEntity:mark_deleted()
  self._deleted = true
end


function CartoonEntity:deleted()
  return true == self._deleted
end


function CartoonEntity:data_set(args)
  if args ~= nil then
...
function CartoonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CartoonEntity:match_set(args)
  if args ~= nil then
...
function CartoonEntity:match_get()
...
function CartoonEntity:stream(action, args, callopts)
...
function CartoonEntity:_run_op(ctx, post_done)
  local utility = self._utility
