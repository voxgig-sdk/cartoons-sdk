...
boru-driven command-line client **and** interactive REPL for the Cartoons
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cartoons-cli)
make build
...
./cartoons-cli list cartoon

# 5. Override the API base URL for a single call
CARTOONS_BASE=https://api.example.com ./cartoons-cli list cartoon
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cartoons-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/cartoons-cli list cartoon
   ```
...
Run with no arguments to open a REPL (prompt `cartoons>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cartoon`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/cartoons-cli`. |
...
(`./cartoons-cli <expr>`) and interactively (the REPL), and expressions compose the
