...
The PHP SDK for the Cartoons API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Cartoon()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/cartoons-sdk/releases](https://github.com/voxgig-sdk/cartoons-sdk/releases)
...
try {
    ...
    foreach ($cartoons as $record) {
        $item = $record->data_get();
        echo $item["id"] . " " . $item["creator"] . "\n";
    }
} catch (\Throwable $err) {
...
print_r(array_map(fn($item) => $item->data_get(), $cartoon));
```
...
```php
$client = CartoonsSDK::test($testopts, $sdkopts);
```
...
| `Cartoon` | `($data): CartoonEntity` | Create a Cartoon entity instance. |
...
```
php/
├── cartoons_sdk.php          -- Main SDK class
...
The main class (`cartoons_sdk.php`) exports the SDK class
...
// $cartoon->data_get() now returns the cartoon data from the last list
// $cartoon->match_get() returns the last match criteria
```
