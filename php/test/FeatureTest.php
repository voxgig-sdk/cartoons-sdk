...
class FtCtrl extends CartoonsControl
{
...
class FtClient
{
    ...
    private CartoonsUtility $utility;

    public function __construct(CartoonsUtility $utility, array $options)
    {
    ...
    public function get_utility(): CartoonsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CartoonsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CartoonsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new CartoonsUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (CartoonsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CartoonsConfig::make_feature($name);
    ...
    private static function buildUrl(CartoonsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CartoonsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CartoonsResponse($fetched);
        }
    ...
    private function fail(CartoonsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CartoonsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
