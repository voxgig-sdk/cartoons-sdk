...
class CartoonsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CartoonsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CartoonsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CartoonsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CartoonsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CartoonsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CartoonsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CartoonsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CartoonsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CartoonsError($op . "_allow",
                "CartoonsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CartoonsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CartoonsError("graphql_error",
                "CartoonsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Cartoon($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_cartoon === null) {
                $this->_cartoon = new CartoonEntity($this, null);
            }
            return $this->_cartoon;
        }
        return new CartoonEntity($this, $data);
    }
