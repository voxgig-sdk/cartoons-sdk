...
[MCP](https://modelcontextprotocol.io) server exposing the Cartoons SDK as
two agent tools — `cartoons_list` and `cartoons_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cartoons-mcp)
make build

# 2. Provide credentials via the environment
export CARTOONS_APIKEY=sk_live_xxx
...
```jsonc
// cartoons_list: first page of records
{ "entity": "cartoon" }
{ "entity": "cartoon", "query": { } }

// cartoons_load: one record by id
{ "entity": "cartoon", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cartoons-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user cartoons \
     -- "$PWD"/dist/*/cartoons-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `cartoons_list` and `cartoons_load` tools now appear
   in new sessions. Ask the agent to *"list cartoon using cartoons"*
   and it calls `cartoons_list` with `{"entity":"cartoon"}`.
...
```sh
export CARTOONS_APIKEY=sk_live_xxx            # API key
export CARTOONS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `cartoons_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `cartoons_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CARTOONS_APIKEY` | API key sent with every request. |
| `CARTOONS_BASE` | Optional override of the API base URL. |
