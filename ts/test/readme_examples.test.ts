...
//     (\`new CartoonsSDK(...)\` / \`CartoonsSDK.test(...)\` become
//     \`CartoonsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"cartoon":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CartoonsSDK(...)\` and any
// \`CartoonsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CartoonsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CartoonsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
