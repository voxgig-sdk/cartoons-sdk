...
class CartoonEntity
  ...
  def make
    opts = @_entopts.dup
    CartoonEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CartoonsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Cartoon, Hash] the current Cartoon data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CartoonsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Cartoon fields)
  def match_get
  ...
  #
  # @param reqmatch [CartoonListMatch, Hash, nil] match filter (any subset of
  #   Cartoon fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Cartoon>, Array] the matching Cartoon items; raises CartoonsError on failure
