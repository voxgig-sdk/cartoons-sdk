...
module CartoonsUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = CartoonsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CartoonsResponse.new(fetched)
    else
