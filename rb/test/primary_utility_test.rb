...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = CartoonsOmni.make_runner(TEST_JSON_FILE, CartoonsSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : CartoonsOmni.errify(err)
    end
