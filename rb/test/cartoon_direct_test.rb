...
class CartoonDirectTest < Minitest::Test
  def test_direct_list_cartoon
    setup = cartoon_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-cartoon", setup[:live] ? "live" : "unit")
    if _should_skip
...
def cartoon_direct_setup(mockres)
  ...
  if live
    ...
    client = CartoonsSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
