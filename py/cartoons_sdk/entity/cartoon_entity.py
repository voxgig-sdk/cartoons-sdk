...
from cartoons_sdk.cartoons_types import (
    Cartoon,
    CartoonListMatch,
)
...
class CartoonEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CartoonEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Cartoon]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Cartoon().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
