...
class TestCartoonEntity:
    ...
    def test_should_stream(self):
        ...
        base = CartoonsSDK.test(seed, None)
        seen = list(base.Cartoon(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = CartoonsSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Cartoon(None).stream("list", None, None):
