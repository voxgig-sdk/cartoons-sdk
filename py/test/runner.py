# Cartoons SDK test runner

from __future__ import annotations
import os
import json

from cartoons_sdk.utility.voxgig_struct import voxgig_struct as vs


class CartoonsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CartoonsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CartoonsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CartoonsTestRunner.getenv("CARTOONS_TEST_LIVE")
        override = CartoonsTestRunner.getenv("CARTOONS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CartoonsTestRunner.getenv(key)
        ...
        explain = CartoonsTestRunner.getenv("CARTOONS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CARTOONS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CartoonsTestRunner._test_control is not None:
            return CartoonsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CartoonsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CartoonsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CartoonsTestRunner.load_test_control()
...
def load_env_local():
    CartoonsTestRunner.load_env_local()


def env_override(m):
    return CartoonsTestRunner.env_override(m)


def entity_data(v):
    return CartoonsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CartoonsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CartoonsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CartoonsTestRunner.load_test_control()


def live_client_options():
    return CartoonsTestRunner.live_client_options()


def live_delay_ms():
    return CartoonsTestRunner.live_delay_ms()
