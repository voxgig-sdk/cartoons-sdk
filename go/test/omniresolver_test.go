...
import (
	...
	omni "github.com/voxgig-sdk/cartoons-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CartoonsError); is {
				out := map[string]any{
					"name":    "CartoonsError",
...
func omniCtx(arg any, client *sdk.CartoonsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CartoonsSDK); is {
			client = live
